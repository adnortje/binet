...
Python Training Script
    ...
    Ref:
        train       : https://pytorch.org/tutorials/beginner/blitz/cifar10_tutorial.html
"""
...
parser.add_argument(
    ...
    type=str,
    required=True,
    choices=['BINetAR', 'BINetOSR', 'MaskedBINet', 'SINet'],
...
elif args.sys == 'BINetOSR':

    sys = inp_net.BINetOSR(
        itrs=args.itrs,
        p_s=args.patch_size,
        b_n=args.bits
    )

elif args.sys == 'MaskedBINet':

    sys = inp_net.MaskedBINet(
        p_s=args.patch_size,
        b_n=args.bits
    )
...
# def optimizer
if args.sys == 'SINet':
    optm = optim.Adam(sys.inp_net.parameters(), args.learn_rate)
else:
    optm = optim.Adam(sys.parameters(), args.learn_rate)
